...
import es.in2.wallet.api.service.impl.AuthorizationRequestServiceImpl;
...
import static es.in2.wallet.api.util.MessageUtils.HEADER_AUTHORIZATION;
...
public class AuthorizationRequestServiceImplTest {
    ...
    void getAuthorizationRequestFromVcLoginRequestTest() {
        try (MockedStatic<ApplicationUtils> ignored = Mockito.mockStatic(ApplicationUtils.class)) {
            ...
            List<Map.Entry<String, String>> headers = List.of(Map.entry(HEADER_AUTHORIZATION, BEARER + authorizationToken));

            when(getRequest(qrContent,headers)).thenReturn(Mono.just("response"));
    ...
    void getAuthorizationRequestFromVcLoginRequestErrorTest() {
        try (MockedStatic<ApplicationUtils> ignored = Mockito.mockStatic(ApplicationUtils.class)) {
            ...
            when(getRequest(qrContent,headers)).thenReturn(Mono.error(new RuntimeException()));
